...
const querySchema = yup.object().shape({
    ...
    filterIdProfessional: yup.number().notRequired().integer(),
    filterIdClient: yup.number().notRequired().integer(),
    filterIdHorario: yup.number().notRequired().integer(),
    filterIdStatus: yup.number().notRequired().integer(),
    filterIdPaymentStatus: yup.number().notRequired().integer()
})
...
export const getAll = async (req: Request<{},{},{}, IQueryProps>, res: Response) => {
    ...
    const result = await AgendamentosProvider.getAll(req.query.page || 1, req.query.limit || 10, req.query.filterIdClient || 0, req.query.filterIdHorario || 0, req.query.filterIdStatus || 0, req.query.filterIdPaymentStatus || 0, Number(req.query.id))
