...
export const seed = async(knex: Knex) => {
    ...
    const [{count}] = await knex(ETableNames.municipio).count<[{count: number}]>('* as count')
    
    if (!Number.isInteger(count) || Number(count) > 0) return
    
    const numeroMunicipios: number = municipiosData.length
    const numeroBlocosInsercao: number = Math.trunc((numeroMunicipios/400))

    let i: number = 0
    while (i <= numeroBlocosInsercao) {
        const bloco = municipiosData.slice((400*i)+1, (400*i)+401)

        const municipioToInsert = bloco.map((municipio) => ({id: municipio.Codigo, name: municipio.Nome, state: municipio.Uf}))
        await knex(ETableNames.municipio).insert(municipioToInsert)
